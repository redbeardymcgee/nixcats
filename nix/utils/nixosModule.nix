...
# Licensed under the MIT license
{
  ...
  config = let
    dependencyOverlays =
      ...
      then pkgs.overlays ++ [(utils.mergeOverlayLists oldDependencyOverlays options_set.addOverlays)]
      else pkgs.overlays ++ options_set.addOverlays;

    mapToPackages = options_set: dependencyOverlays: (
      let
        newCategoryDefinitions =
          if options_set.categoryDefinitions.replace != null
        ...
        pkgDefs =
          if (options_set.packages != null)
          then packageDefinitions // options_set.packages
          else packageDefinitions;

        newLuaBuilder =
          if options_set.luaPath != ""
          then (utils.baseBuilder options_set.luaPath)
          else
            (
              if keepLuaBuilder != null
              then keepLuaBuilder
              else builtins.throw "no lua or keepLua builder supplied to mkNixosModules"
            );

        newNixpkgs =
          if config.${defaultPackageName}.nixpkgs_version != null
