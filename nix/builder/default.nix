...
# Licensed under the MIT license
{
  ...
  pkgs = with builtins;
    if
      isAttrs nixCats_passthru
      && isAttrs extra_pkg_config
...
in let
  ...
  nixCats = {...} @ allPluginDeps:
    pkgs.stdenv.mkDerivation (let
      ...
      nixCats_config_location =
        if isUnwrappedCfgPath
        then "${settings.unwrappedCfgPath}"
        else if isStdCfgPath
        then ncTools.mkLuaInline ''vim.fn.stdpath("config")''
        else "${LuaConfig}";
  ...
  customRC = let
    optLuaPre =
      if builtins.isString optionalLuaPreInit
      then optionalLuaPreInit
      else
        builtins.concatStringsSep "\n"
        (pkgs.lib.unique (filterAndFlatten optionalLuaPreInit));
    optLuaAdditions =
      if builtins.isString optionalLuaAdditions
  ...
  combineCatsOfFuncs = section: (x: let
  ...
  extraMakeWrapperArgs = let
    ...
    preORpostPATH =
      if settings.suffix-path
      then "suffix"
      else "prefix";
    preORpostLD =
      if settings.suffix-LD
      then "suffix"
      else "prefix";
  in
    builtins.concatStringsSep " " (
      ...
      (
        ...
        then [''--set NVIM_APPNAME "${settings.configDirName}"'']
        else []
      )
  ...
  python3wrapperArgs = pkgs.lib.unique ((filterAndFlatten extraPython3wrapperArgs)
    ++ (
      if settings.disablePythonSafePath
      then ["--unset PYTHONSAFEPATH"]
      else []
    ));

  preWrapperShellCode =
    if builtins.isString bashBeforeWrapper
  ...
  baseNvimUnwrapped =
    if settings.neovim-unwrapped == null
    then pkgs.neovim-unwrapped
    else settings.neovim-unwrapped;
  myNeovimUnwrapped = baseNvimUnwrapped.overrideAttrs (prev: {
    src =
      if settings.nvimSRC != null
      then settings.nvimSRC
      else prev.src;
