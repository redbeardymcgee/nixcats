...
assert withPython2 -> throw "Python2 support has been removed from the neovim wrapper, please remove withPython2 and python2Env."; let
  generateProviderRc = {
    withPython3 ? true,
    withNodeJs ? false,
    withRuby ? true,
    ...
    genProviderCommand = prog: withProg:
      if withProg
      then "vim.g.${prog}_host_prog='${placeholder "out"}/bin/${nixCats_packageName}-${prog}'"
  ...
  stdenv.mkDerivation {
    ...
    meta =
      ...
      // {
        ...
        maintainers =
          neovim-unwrapped.meta.maintainers
          ++ (
