...
# https://github.com/NixOS/nixpkgs/blob/8564cb1517f118e1e90b8bc9ba052678f1aa4603/pkgs/applications/editors/neovim/utils.nix#L126-L164
{
  ...
  parsepluginspec = opt: p: let
    optional =
      if p ? optional && builtins.isBool p.optional
      then p.optional
      else opt;
  ...
  lcfgs = builtins.filter (v: v != null) (map (v:
    if v ? type && v.type == "lua"
    then v.config
    else null)
  pluginsWithConfig);
  vcfgs = builtins.filter (v: v != null) (map (v:
    if v ? type && v.type == "viml"
    then v.config
    else null)
  pluginsWithConfig);
...
in
  (pkgs.callPackage ./wrapper.nix {}) neovim-unwrapped (
    res
    // {
