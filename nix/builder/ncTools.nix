with builtins; rec {
  ...
  toLuaInternal = {
    ...
    in
      ...
      then "${luaListPrinter value level}"
      else if lib.isDerivation value
      then luaEnclose "${value}"
      else if isLuaInline value
      then replacer (luaToString value)
      else if isAttrs value
      then "${luaTablePrinter value level}"
    ...
    luaTablePrinter = attrSet: level: let
      luatableformatter = attrSet: let
        nameandstringmap = mapAttrs (n: value: let
          name = "[ " + (luaEnclose "${n}") + " ]";
        in "${name} = ${doSingleLuaValue (level + 1) value}")
        attrSet;
  ...
  RecFilterCats = categories: categoryDefs: let
    ...
    filterLayer = attr: (lib.filterAttrs (
        name: value:
          if isBool value
          then value
  ...
  flattenToList = attrset:
    concatMap
    (
      v:
        if isAttrs v && !lib.isDerivation v
        then flattenToList v
        else if isList v
        then v
        else if v != null
        then [v]
