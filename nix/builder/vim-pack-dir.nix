...
# https://github.com/NixOS/nixpkgs/blob/ae5d2af73efa5e25bf9bf43672cd3d8d99c613d0/pkgs/applications/editors/vim/plugins/vim-utils.nix#L136-L207
{
  ...
  callNixCats = nixCats: {
    ...
    ts_grammar_path =
      if isOldGrammarType
      then ts_grammar_plugin_combined
      else mkLuaInline "vim.g[ [[nixCats-special-rtp-entry-vimPackDir]] ] .. [[/pack/${grammarPackName}/start/*]]";
    ...
    fullDeps = {
      ...
      python3Path =
        if (allPython3Dependencies python3.pkgs == [])
        then null
  ...
  in
    # we add the plugin with ALL the parsers if its the old way, if its the new way, it will be in our packpath already
